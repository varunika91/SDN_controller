...
class l3_switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      if packet.find("icmp"):
        if((packet.next.dstip == IPAddr("10.0.%d.1" %dpid))):
            log.debug("switch replying to icmp")
            # Make the ping reply
            icmp = pkt.icmp()
      ...
      if dstaddr in self.arpTable[dpid]:
        ...
        log.debug("In if dstaddr in self.arpTable[dpid]:")
        prt = self.arpTable[dpid][dstaddr].port
        mac = self.arpTable[dpid][dstaddr].mac
        if prt == inport:
          log.warning("%i %i not sending packet for %s back out of the " + "input port" % (dpid, inport, str(dstaddr)))
        else:
          log.debug("%i %i installing flow for %s => %s out port %i, dstaddr in arptable"
        ...
        msg.in_port = inport
        event.connection.send(msg)
        host_topology=["10.0.2.1","10.0.1.2","10.0.1.3","10.0.2.2","10.0.1.1","10.0.2.3","10.0.2.4"]
        if (packet.find("icmp")):
			if(packet.find("ipv4").dstip not in host_topology):
				icmp = pkt.icmp()
      ...
      if a.prototype == arp.PROTO_TYPE_IP:
        if a.hwtype == arp.HW_TYPE_ETHERNET:
          if a.protosrc != 0:
            ...
            if a.opcode == arp.REQUEST and a.protodst.inNetwork("10.0.%d.0/24" %dpid):
                r = arp()
                r.hwtype = a.hwtype
                ...
                e = ethernet(type=packet.type, src=dpid_to_mac(dpid),
                           dst=a.hwsrc)
                e.set_payload(r)
                log.debug("%i %i answering ARP for %s with hwsrc %s" % (dpid, inport,
                str(r.protosrc), (str(a.hwsrc))))
                msg = of.ofp_packet_out()
                msg.data = e.pack()
